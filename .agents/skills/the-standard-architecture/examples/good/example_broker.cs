...
namespace MyProject.Brokers.Storages
{
    ...
    public partial class StorageBroker : EFxceptionsContext, IStorageBroker
    {
        ...
        private ValueTask<IQueryable<T>> SelectAllAsync<T>()
            where T : class => ValueTask.FromResult<IQueryable<T>>(this.Set<T>().AsNoTracking());

        private async ValueTask<T> SelectAsync<T>(Guid entityId)
            where T : class => await this.FindAsync<T>(entityId);
