...
- [x] **Secret Management & Startup Validation:**
  - Use **direnv** (leveraging **Infisical** with `.env.local` overrides) as the single secret delivery mechanism for both .NET and Nuxt — replaces `dotnet user-secrets` and avoids `.env` file sprawl.
  - Secrets are loaded from Infisical first, then `.env.local` acts as an override layer.
